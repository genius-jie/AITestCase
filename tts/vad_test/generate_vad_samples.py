...
def detect_language(text):
    """
    检测文本语言，返回语言代码和对应的语音名称
    
    Args:
        text: 待检测的文本内容
        
    Returns:
        tuple: (lang_code, voice_name)
    ...
    if has_english:
        # 英语语音
        return "en-US", "en-US-JennyNeural"
    else:
        # 中文语音（使用配置文件中的默认语音）
        return "zh-CN", VOICE_NAME
...
def split_mixed_text(text):
    ...
    Returns:
        list: 片段列表，每个片段是包含文本和语言类型的字典
              [{"text": "你好", "lang": "zh-CN", "voice": "zh-CN-XiaoxiaoNeural"},
               {"text": "Hello", "lang": "en-US", "voice": "en-US-JennyNeural"}]
    """
    segments = []
    current_segment = ""
    current_lang = None
    ...
    while i < len(text):
        ...
        # 判断字符语言类型
        is_chinese = '\u4e00' <= char <= '\u9fff'  # 中文字符范围
        is_english = char.isalpha() and char.isascii()  # 英文字母
        is_number = char.isdigit()  # 数字
        ...
        if is_chinese:
            ...
            # 数字跟随前一个语言，或者默认为中文
            char_lang = current_lang if current_lang else "zh-CN"
        else:
            # 其他字符（标点等）跟随前一个语言
            char_lang = current_lang if current_lang else "zh-CN"
        ...
        if current_lang is None:
            ...
            # 语言切换，保存当前片段，开始新片段
            if current_segment.strip():  # 只保存非空片段
                segments.append({
                    "text": current_segment,
                    "lang": current_lang,
                    "voice": VOICE_NAME if current_lang == "zh-CN" else "en-US-JennyNeural"
                })
            current_lang = char_lang
            current_segment = char
...
def generate_ssml(text, break_time_ms=None, pause_position=None, 
    ...
    is_mixed_language = len(segments) > 1
    ...
    # 开始构建SSML
    # 对于混合语言，使用第一个片段的语言作为主语言
    main_lang = segments[0]["lang"] if segments else "zh-CN"
    ssml_parts = [f"<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='{main_lang}' xmlns:mstts='http://www.w3.org/2001/mstts'>"]
    
    # 处理停顿和杂音
    pause_elements = []
    if break_time_ms:
        if pause_position == "句首":
            pause_elements.append(f"<break time='{break_time_ms}ms'/>")
            if add_noise:
                pause_elements.append(f"<audio src='{noise_url}'/>")
    ...
    for i, segment in enumerate(segments):
        ...
        if prosody_attrs:
            segment_content = f"<prosody {' '.join(prosody_attrs)}>{segment_content}</prosody>"
        
        # 在第一个片段前添加句首停顿
        if i == 0 and pause_position == "句首":
            ssml_parts.extend(pause_elements)
        
        ssml_parts.append(segment_content)
        
        # 在最后一个片段后添加句尾停顿
        if i == len(segments) - 1 and pause_position == "句尾":
            ssml_parts.extend(pause_elements)
        # 在中间片段添加句中停顿
        elif i < len(segments) - 1 and pause_position == "句中":
            ssml_parts.extend(pause_elements)
