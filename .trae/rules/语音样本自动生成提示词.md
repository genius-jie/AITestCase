...
4. **命名规范**：建议使用 `vad_test_data_v{version}.csv` 和 `SC{id}_{description}.wav` 格式。
...
在生成语音样本时，必须遵循以下规则以避免产生幻觉：

### 1. 数据源验证
*   **必须**基于用户明确描述的需求生成语音样本
*   **严禁**编造用户未要求的情绪、语速、停顿等参数
*   **必须**在生成CSV文件时严格遵循21个字段的格式要求
*   **严禁**添加CSV格式要求中不存在的字段

### 2. 知识边界明确
*   **必须**仅使用Azure TTS支持的参数和选项
*   **严禁**使用TTS引擎不支持的特性或参数
*   **必须**在不确定参数有效性时参考Azure TTS官方文档
*   **严禁**基于其他TTS引擎的经验生成Azure TTS配置

### 3. 禁止臆造
*   **必须**确保每个测试场景都有明确的需求依据
*   **严禁**编造不存在的情绪风格、语速选项或杂音类型
*   **必须**准确反映用户描述的测试需求
*   **严禁**为了增加样本数量而虚构测试场景

### 4. 引用规范
*   **必须**在生成CSV文件时使用准确的字段名和枚举值
*   **严禁**凭记忆或推测使用字段名或枚举值
*   **必须**严格遵循CSV格式要求中的示例格式
*   **严禁**臆造字段值或修改枚举选项

### 5. 事实核查
*   **必须**在生成语音样本前核查CSV文件的完整性和准确性
*   **严禁**生成格式错误或参数无效的CSV文件
*   **必须**确保所有必需字段都有有效值
*   **严禁**为了满足样本数量要求而生成无效的测试用例

### 6. 版本兼容性
*   **必须**在涉及Azure TTS API时确认当前版本支持的特性
*   **严禁**使用过时或不支持的API参数
*   **必须**确保生成的配置与azure_config.py中的设置一致
*   **严禁**基于通用知识补充对话中未提及的技术细节

### 7. 参数有效性验证
*   **必须**在设置break_time_ms时验证范围在100-5000ms之间
*   **严禁**设置超出范围的停顿时长
*   **必须**在设置style字段时使用有效的情绪风格枚举值
*   **严禁**使用未在支持列表中的情绪风格
*   **必须**在设置prosody_rate时使用fast/normal/slow之一
*   **严禁**使用其他语速选项
...
### 1. 错题本机制 (Error Log)
*   **触发条件**：当语音生成失败、CSV 格式错误或参数配置无效时。
*   **执行动作**：必须将该 Case 记录到 `e:\AI测试用例\.trae\rules\错题本规则.md` 的"历史错误记录"章节。
*   **记录格式**：
    ```markdown
    | 日期 | 模块 | 错误描述 | 根因分析 | 修正方案 |
    | :--- | :--- | :--- | :--- | :--- |
    | 2024-01-11 | VAD | 语音生成失败 | Azure TTS Key 无效 | 检查 azure_config.py 配置 |
    ```

### 2. 提示词自迭代 (Prompt Optimization)
*   **触发条件**：当 `错题本规则.md` 中连续出现 3 次同类 VAD/TTS 相关错误。
*   **执行动作**：
    1.  **分析**：确认是 Prompt 中的参数说明有误，还是示例代码过时。
    2.  **建议**：向用户主动提出修改 System Prompt 的建议。
    3.  **更新**：获得用户许可后，更新 `语音样本自动生成提示词.md`。
