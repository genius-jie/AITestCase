...
### 2026-01-11 - CSVDataSet quotedData配置错误导致JSON字段解析失败

**问题描述**
执行JMeter脚本时报错：
```
ERROR o.a.j.c.CSVDataSet: java.io.IOException: Cannot have quote-char in plain field:[[{"]
```

**原因分析**
1. **CSV文件包含JSON数据**：CSV文件的`history`字段包含JSON数据，JSON中有引号（`"`）
2. **quotedData配置错误**：CSVDataSet组件中设置了`quotedData="true"`
3. **解析冲突**：当`quotedData="true"`时，JMeter会尝试解析引号作为字段分隔符的一部分，但CSV文件使用`|`作为分隔符，字段内的引号应该被当作普通字符处理

**解决方案**
将CSVDataSet组件的`quotedData`属性从`true`改为`false`：

```xml
<!-- 修复前 -->
<CSVDataSet guiclass="TestBeanGUI" testclass="CSVDataSet" testname="CSV数据文件设置">
  <stringProp name="delimiter">|</stringProp>
  <stringProp name="fileEncoding">UTF-8</stringProp>
  <stringProp name="filename">e:\AI测试用例\intent_recognition_test_data_clean.csv</stringProp>
  <boolProp name="ignoreFirstLine">true</boolProp>
  <boolProp name="quotedData">true</boolProp>
  <boolProp name="recycle">false</boolProp>
  <boolProp name="stopThread">true</boolProp>
  <stringProp name="shareMode">shareMode.all</stringProp>
  <stringProp name="variableNames">user_id,session_id,history,asr_emotion,asr_emotion_value,asr_emotion_confidence,expected_status,expected_label</stringProp>
</CSVDataSet>
...
**预防措施**
- ✅ CSV文件使用自定义分隔符（如`|`、`;`）时，设置`quotedData="false"`
- ✅ CSV文件字段包含JSON、XML等包含引号的数据时，设置`quotedData="false"`
- ✅ CSV文件使用标准分隔符（`,`）且字段用引号包裹时，才设置`quotedData="true"`
- ✅ 遇到CSV解析错误时，首先检查`quotedData`配置是否与CSV文件格式匹配
- ✅ 使用JMeter GUI创建CSVDataSet组件，学习正确的配置方式

**适用场景**
- JMeter CSV数据文件包含JSON数据
- JMeter CSV数据文件使用自定义分隔符
- JMeter CSV数据文件字段包含引号
- 任何需要从CSV文件读取包含特殊字符的数据的测试场景
