...
#### 宪法级前置条件
**必须首先阅读并理解**：[JMeter脚本设计规范.md](file:///e:/AI测试用例/.trae/rules/agents/jmeter/JMeter脚本设计规范.md) 第0章"使用说明"
...
### 7.1 核心职责与能力需求分析
**A5阶段核心职责**：执行可达性验证、断言设计与上下游变量传递，在真实环境中验证脚本可执行性，完成断言设计，尝试上下游接口关联与变量传递。

### 7.2 针对性Skill能力映射

| 能力类别 | 适用Skill | 能力等级 | 应用场景 | 核心价值 |
|---------|---------|---------|---------|---------|
| **执行验证** | JMeter 执行层设计 | L4 | 执行可达性验证、断言设计、执行调试、结果分析 | 提供完整的执行层能力，确保脚本真正可用，具备断言判断和动态调整能力 |
| **方法封装** | JMeter 方法层设计 | L3 | 上下游关联处理、变量传递、断言配置 | 提供方法封装和变量传递能力，处理接口间依赖关系 |
| **组件配置** | JMeter 组件层设计 | L2 | 脚本工程骨架搭建、GUI/CLI兼容性保证 | 确保脚本结构正确，GUI与CLI模式兼容 |
| **测试数据设计** | 接口测试数据设计 | L1 | 数据策略执行、数据验证 | 严格执行已批准的数据策略，确保数据使用正确 |

### 7.3 阶段专属Skill能力配置

**A5阶段专属能力配置**：
- ✅ **JMeter 执行层设计**：用于执行可达性验证、断言设计、执行调试和结果分析
- ✅ **JMeter 方法层设计**：用于处理上下游接口关联和变量传递
- ✅ **JMeter 组件层设计**：用于脚本工程骨架搭建和兼容性保证
- ⚠️ **接口测试数据设计**：仅用于执行已批准的数据策略，不用于数据策略决策

**能力使用原则**：
- **断言设计优先**：基于Baseline样本设计断言，确保判责明确
- **执行可达性验证**：在真实环境中验证脚本可执行性
- **上下游关联**：尝试完成上下游接口关联与变量传递
- **有限自愈**：单阶段重试≤3次，超过阈值请求人工介入