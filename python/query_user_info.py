...
class DatabaseQuery:
    ...
    def analyze_data(self, user_id: str, redis_data: Dict[str, Any], pg_data: Dict[str, List[Dict[str, Any]]]):
        ...
        if history_key in redis_data:
            history_count = len(redis_data[history_key])
        ...
        # 检查用户记忆
        if pg_data.get("user_memories"):
            print(f"   用户短期记忆: 存在")
        
        if pg_data.get("user_long_memories"):
            # 统计长期记忆分类
            categories = {}
            for memory in pg_data["user_long_memories"]:
                category = memory.get("category", "unknown")
                categories[category] = categories.get(category, 0) + 1
            print(f"   用户长期记忆分类: {list(categories.keys())}")
        
        if pg_data.get("user_long_profile_insights"):
            print(f"   用户分类洞察数: {len(pg_data['user_long_profile_insights'])}")
        
        if pg_data.get("user_long_personas"):
            print(f"   完整用户画像: 存在")
...
if __name__ == "__main__":
    ...
    # 执行查询
    query.run_query(default_user_id)
